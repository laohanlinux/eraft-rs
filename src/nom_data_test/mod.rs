...
pub fn walk<F>(path: &str, mut f: F) where F: FnMut(&str) {
...
pub fn execute_test<P: AsRef<Path>, F>(path: P, split: &str, mut f: F) where F: FnMut(&TestData) -> String {
    let mut data = vec![];
        ...
        while let Ok(n) = rd.read_line(&mut buf) {
            ...
            if buf.starts_with("title: ") {
                ...
                for arg in args.split_terminator(" ").collect::<Vec<_>>() {
                    let mut cmd_arg = CmdArg { key: "".to_string(), vals: vec![] };
                    let arg = arg.split("=").collect::<Vec<_>>();
                    cmd_arg.key = arg[0].to_string();
                    cmd_arg.vals = arg[1].trim_start_matches('(').trim_end_matches(')').split(",").filter(|s| s.trim() != "").map(|s| s.to_string()).collect::<Vec<_>>();
                    cmd.cmd_args.push(cmd_arg);
                }
            } else if buf.starts_with("output:") {} else {
